...
public class ${className}Controller extends BaseController{
	...
	@ResponseBody
	public JSONObject datagrid(@ModelAttribute ${className}Entity query, HttpServletRequest request, HttpServletResponse response,
	...
	public AjaxJson doAdd(@ModelAttribute ${className}Entity ${lowerName},
		...
		try {
			...
			for (${sub.paramData.className}Entity entity : ${sub.paramData.lowerName}Entities) {
				...
				if(StringUtils.isEmpty(${lowerName}.get${sub.paramData.mainForeignKeyUpper}())){
			    	j.setMsg("关联外键不能为空");
					j.setSuccess(false);
					return j;
			    }
	...
	public AjaxJson doEdit(@ModelAttribute ${className}Entity ${lowerName},
		...
		try {
			...
			if(!temp${className}Entity.get${sub.paramData.mainForeignKeyUpper}().equals(${lowerName}.get${sub.paramData.mainForeignKeyUpper}())){
				this.${sub.paramData.lowerName}Service.deleteBy${sub.paramData.foreignKeyUpper}(temp${className}Entity.get${sub.paramData.mainForeignKeyUpper}());
			}else{
				List<${sub.paramData.className}Entity> old${sub.paramData.className}EntityList = this.${sub.paramData.lowerName}Service.getBy${sub.paramData.foreignKeyUpper}(${lowerName}.get${sub.paramData.mainForeignKeyUpper}());
	...
	public AjaxJson doDelete(@RequestParam(required = true, value = "id" ) String id){
			...
			try {
				...
				<#list subEntityList as sub>
				this.${sub.paramData.lowerName}Service.deleteBy${sub.paramData.foreignKeyUpper}(${lowerName}.get${sub.paramData.mainForeignKeyUpper}());
				</#list>
				j.setMsg("删除成功");
			} catch (Exception e) {
				j.setSuccess(false);
	 ...
	 * @return
	 */
	@RequestMapping(params = "doBatchDel",method = {RequestMethod.GET, RequestMethod.POST})
	@ResponseBody
	public AjaxJson doBatchDel(String ids, HttpServletRequest request){
		AjaxJson j = new AjaxJson();
		String message = "列表选中数据删除成功";
		try{
			for(String id : ids.split(",")){
				...
				<#list subEntityList as sub>
				List<${sub.paramData.className}Entity> ${sub.paramData.lowerName}Entities = this.${sub.paramData.lowerName}Service.getBy${sub.paramData.foreignKeyUpper}(${lowerName}.get${sub.paramData.mainForeignKeyUpper}());
			    if(${sub.paramData.lowerName}Entities != null && ${sub.paramData.lowerName}Entities.size() > 0) {
			    	for (${sub.paramData.className}Entity entity : ${sub.paramData.lowerName}Entities) {
			    		this.${sub.paramData.lowerName}Service.deleteBy${sub.paramData.foreignKeyUpper}(entity.get${sub.paramData.mainForeignKeyUpper}());
					}
			    }
			    </#list>
			}
