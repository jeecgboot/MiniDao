...
<form class="form-horizontal" role="form" id="formobj" action="$!{basePath}/${projectName}/${lowerName}.do?doEdit" method="POST">
	<input type="hidden" id="btn_sub" class="btn_sub"/>
	<input type="hidden" id="id" name="id" value="$!{${lowerName}.id}"/>
	<fieldset>
		...
		<div class="tab-pane fade" id="${key.paramData.lowerName}">  
			...
			<div class="form-tb-toolbar">
	   		...
	   		<table class="table subinfo-table" id="${key.paramData.lowerName}_table">
				...
				<tbody>
				#set($count=0)
				#if($!{${key.paramData.lowerName}Entities.size()} > 0)
				#foreach( $!{${key.paramData.lowerName}} in $!{${key.paramData.lowerName}Entities})
				<tr>
			  		<td class="form-ck">
			  			<input style="" type="checkbox" name="ck"/>
			  			<input name = "${key.paramData.lowerName}List[$!{count}].id" value= "$!{${key.paramData.lowerName}.id}" type="hidden"/>
			  		</td>
			  	<#list key.paramData.columnDatas as po>
					<#if po.domainPropertyName != 'id'>
					<#if po.columnType == "datetime" || po.columnType == "date" || po.columnType == "timestamp">
					<td>
						<input type="text"  name="${key.paramData.lowerName}Entities[$!{count}].${po.domainPropertyName}" value="$!{${key.paramData.lowerName}.${po.domainPropertyName}}" id="${key.paramData.lowerName}Entities[$!{count}].${po.domainPropertyName}" class="form-control" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'})"   style="background: url('$!{basePath}/plug-in-ui/images/datetime.png') no-repeat scroll right center transparent;" <#if po.nullable != 'Y'> datatype="*" </#if>/>
					 </td>
					<#else>
					<td>
						<input type="text" name="${key.paramData.lowerName}Entities[$!{count}].${po.domainPropertyName}" value="$!{${key.paramData.lowerName}.${po.domainPropertyName}}" id="${key.paramData.lowerName}Entities[$!{count}].${po.domainPropertyName}" class="form-control" <#if po.nullable != 'Y'> datatype="*" </#if>/>
					 </td>
					</#if>
					</#if>
				</#list>
				...
				<tr>
				  	...
				  	<#if po.domainPropertyName != 'id'>
						<#if po.columnType == "datetime" || po.columnType == "date" || po.columnType == "timestamp">
						<td>
							<input type="text"  name="${key.paramData.lowerName}Entities[0].${po.domainPropertyName}" value="$!{${key.paramData.lowerName}.${po.domainPropertyName}}" id="${key.paramData.lowerName}Entities[0].${po.domainPropertyName}" class="form-control" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'})"   style="background: url('$!{basePath}/plug-in-ui/images/datetime.png') no-repeat scroll right center transparent;" <#if po.nullable != 'Y'> datatype="*" </#if>/>
						 </td>
						<#else>
						<td>
							<input type="text" name="${key.paramData.lowerName}Entities[0].${po.domainPropertyName}" value="$!{${key.paramData.lowerName}.${po.domainPropertyName}}" id="${key.paramData.lowerName}Entities[0].${po.domainPropertyName}" class="form-control" <#if po.nullable != 'Y'> datatype="*" </#if>/>
						 </td>
						</#if>
					</#if>
					</#list>
